...
import {
    ...
    VoteType,
} from '../modules/governance/models/governance.proposal.model';
import { registerEnumType } from '@nestjs/graphql';
import { GOVERNANCE_EVENTS } from '@terradharitri/sdk-exchange';
...
registerEnumType(GovernanceSmoothingFunction, {
    name: 'GovernanceSmoothingFunction',
});
...
export const governanceSmoothingFunction = (
    governanceAddress: string,
): GovernanceSmoothingFunction | undefined => {
    const types = Object.keys(governanceConfig);
    for (const type of types) {
        const smoothingFunctions = Object.keys(governanceConfig[type]);
        for (const smoothingFunction of smoothingFunctions) {
...
export const toGovernanceProposalStatus = (
    status: string,
): GovernanceProposalStatus => {
    switch (status) {
        case 'None':
