...
import { GOVERNANCE_EVENTS, VoteEvent } from '@terradharitri/sdk-exchange';
...
export class GovernanceHandlerService {
    ...
    async handleGovernanceVoteEvent(
        ...
        const userVotedProposals = await this.governanceAbi.userVotedProposals(
            event.address,
            topics.voter,
        );
        ...
        const userVotedProposalsWithVoteType =
            await this.governanceCompute.userVoteTypesForContract(
                event.address,
                topics.voter,
            );
        const cachedVoteType = userVotedProposalsWithVoteType.find(
            (proposal) => proposal.proposalId === topics.proposalId,
        );
        if (!cachedVoteType) {
    ...
    async increaseProposalVotes(
        event: VoteEvent,
        voteType: GOVERNANCE_EVENTS,
    ): Promise<ProposalVotes> {
        const topics = event.getTopics();
        const proposalVotes = await this.governanceAbi.proposalVotes(
            event.address,
            topics.proposalId,
        );

        proposalVotes.totalVotes = new BigNumber(proposalVotes.totalVotes)
            .plus(topics.nrVotes)
            .toFixed();
        proposalVotes.quorum = new BigNumber(proposalVotes.quorum)
            .plus(topics.quorumUsed)
            .toFixed();
        switch (voteType) {
            case GOVERNANCE_EVENTS.UP:
                proposalVotes.upVotes = new BigNumber(proposalVotes.upVotes)
                    .plus(topics.nrVotes)
                    .toFixed();
                break;
            case GOVERNANCE_EVENTS.DOWN:
                proposalVotes.downVotes = new BigNumber(proposalVotes.downVotes)
                    .plus(topics.nrVotes)
                    .toFixed();
                break;
            case GOVERNANCE_EVENTS.ABSTAIN:
                proposalVotes.abstainVotes = new BigNumber(
            ...
            case GOVERNANCE_EVENTS.DOWN_VETO:
                proposalVotes.downVetoVotes = new BigNumber(
                    proposalVotes.downVetoVotes,
                )
                    .plus(topics.nrVotes)
                    .toFixed();
                break;
        }
        proposalVotes.upPercentage = new BigNumber(proposalVotes.upVotes)
            .dividedBy(proposalVotes.totalVotes)
            .multipliedBy(100)
            .toFixed(2);
        proposalVotes.downPercentage = new BigNumber(proposalVotes.downVotes)
            .dividedBy(proposalVotes.totalVotes)
            .multipliedBy(100)
            .toFixed(2);
        proposalVotes.abstainPercentage = new BigNumber(
            proposalVotes.abstainVotes,
        )
            .dividedBy(proposalVotes.totalVotes)
            .multipliedBy(100)
            .toFixed(2);
        proposalVotes.downVetoPercentage = new BigNumber(
            proposalVotes.downVetoVotes,
        )
            .dividedBy(proposalVotes.totalVotes)
            .multipliedBy(100)
