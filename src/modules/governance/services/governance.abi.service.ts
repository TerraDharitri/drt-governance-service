...
export class GovernanceTokenSnapshotAbiService extends GenericAbiService {
    ...
    async proposalsRaw(scAddress: string): Promise<GovernanceProposalModel[]> {
        ...
        return response.firstValue.valueOf().map((proposal: any) => {
            ...
            if (
                scAddress ===
                'drt1qqqqqqqqqqqqqpgq8xqp6c0kzwn3f2c5zsxfex6h69s2x9rwhg4sxjct2x'
            ) {
            ...
            if (
                scAddress ===
                    'drt1qqqqqqqqqqqqqpgqfn2mu8l0dte34eqh6qtgmpjpxpkhunccrl4sek8zvq' &&
                proposal.proposal_id.toNumber() === 1
            ) {
                description = JSON.stringify({
                    title: 'Andromeda 1.9 Protocol Upgrade',
                    shortDescription:
                        'Andromeda is the first major upgrade in a two-step roadmap to drastically reduce transaction time to finality on DharitrI. This release redesigns consensus mechanisms, finalization rules, and cross-shard execution, significantly improving the efficiency, security, and scalability of the network.',
                    strapiId: 8,
                    version: 1,
                });
            }
