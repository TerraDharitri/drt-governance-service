...
export class GovernanceTokenSnapshotMerkleService {
    ...
    async getMerkleTree(rootHash: string): Promise<MerkleTreeUtils> {
        return (
    ...
    async getAddressBalance(
        ...
        return (
            merkleTree.getLeaves().find((leaf) => leaf.address === address)
                ?.balance ?? '0'
        );
    }

    private async createMerkleTree(rootHash: string): Promise<MerkleTreeUtils> {
        const jsonContent: string = await promises.readFile(
            `./src/snapshots/${rootHash}.json`,
            {
                encoding: 'utf8',
            },
        );
        ...
        if (newMT.getRootHash() !== `0x${rootHash}`) {
            throw new Error(
                "Computed root hash doesn't match the provided root hash.",
            );
        }
